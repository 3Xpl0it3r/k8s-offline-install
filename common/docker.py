...
class DockerInstall:
    ...
    def install_from_binary(host, password):
        ...
        list(map(lambda _: LocalCommand.scp(host, password, _, os.path.join("/usr/bin/", os.path.basename(_))), allfiles))
        list(map(lambda _: RemoteCommand.security_command(host, password, "chmod +x /usr/bin/{}".format(os.path.basename(_))), allfiles))
